...
public:
  ...
  bool connect_uinput_server() {
    ...
    char time_suffix[32];
    // Định dạng: NgàyThángNăm (Ví dụ: 31012026)
    strftime(time_suffix, sizeof(time_suffix), "%d%m%Y", ltm);

    // 2. Tạo tên Socket tàng hình độc bản của Thành
    // Thêm chữ "thanhpy" để đánh dấu bản quyền trong RAM
    ...
    // Lớp 1: Tàng hình (Abstract Socket)
    // Ký tự đầu tiên phải là \0
    addr.sun_path[0] = '\0'; 
    // Copy tên socket vào sau ký tự \0
    strncpy(addr.sun_path + 1, socket_name.c_str(), sizeof(addr.sun_path) - 2);

    // Lớp 2: Kết nối (Client không cần quan tâm file ở đâu, chỉ cần tìm trong Kernel)
